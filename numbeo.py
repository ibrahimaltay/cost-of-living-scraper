...
def CompareCountries(Ct1,Ct2):
	Country1 = Ct1
	Country2 = Ct2
	
	URL = f"https://www.numbeo.com/cost-of-living/compare_countries_result.jsp?country1={Country1}&country2={Country2}&displayCurrency={CURRENCY}"
	source = requests.get(URL).text
	soup = BeautifulSoup(source,'lxml')
	...
	for x in row:
		try:
			items.append(x.find_all('td')[0].text)
			prices1.append(x.find_all('td')[1].text)
			prices2.append(x.find_all('td')[2].text)
		except Exception as e:
			pass
	result = list(zip(items,prices1,prices2))
	return result

def WriteCSV(data,name):
	with open(f'{name}.csv','w', newline='') as file:
		writer = csv.writer(file)
		writer.writerow(["ITEMS","PRICES"])
		for x in data:
			writer.writerow([x[0],x[1].split("$")[0]])
...
# denemelist = ["Turkey","Japan","Germany","Russia"]

data = {
 	'item' : [ITEMS],
 	'Turkey' : [GetCountryPrices("Turkey")]
}
...
#	data = {
#	'item' : [ITEMS],
#	country : [GetCountryPrices(country)]
#	}
